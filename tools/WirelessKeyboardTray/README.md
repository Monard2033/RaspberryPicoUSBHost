...
wireless Battery packet. Left-click and right-click both open the native modern
menu that shows Battery percentage, voltage, charge state, telemetry state/age
and sequence above the action items. The menu items include `Refresh now`,
`Start with Windows`, and `Exit`. The icon is green/yellow/red according to level,
