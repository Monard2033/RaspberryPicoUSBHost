...
def main():
    ...
    while offset < total_size:
        ...
        # Stream 5-byte chunks, each labeled with a sequence byte (data[7])
        # derived from the chunk index. The RP2040 accepts only the exact
        # next sequence (mod 256) and drops duplicates/out-of-order chunks,
        # making re-sends idempotent. Pacing: ~2 KB/s is the stable ESB
        # ACK-payload throughput (dongle drains one payload per radio
        # packet); faster writes overflow the ACK queue and lose chunks.
            ...
            if st is not None:
                ...
                if s_session == session:
                    ...
                    if (time.time() - last_progress) > 2.0:
                        ...
                        # Re-send exactly from the device count. The chunk
                        # sequence is derived from the byte offset (5 bytes
                        # per chunk), so re-sent chunks carry the exact
                        # sequences the device expects next — duplicates and
                        # out-of-order chunks are dropped device-side.
                        p = val
