...
5. If step 4 returns 401, confirm a credential actually exists before troubleshooting auth: `cmdkey /list | Select-String github` shows the GCM entry, then repeat step 3. Completion: either the PR is created or you can report the credential store as the concrete blocker.
