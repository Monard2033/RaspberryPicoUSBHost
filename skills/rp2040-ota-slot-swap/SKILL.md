...
4. Restore interrupts, then reboot with `watchdog_reboot(0, 0, 0)`. Completion check: no XIP call exists between the first erase and the reboot; grep the swap function for `watchdog_update|printf|LOG`.
